...
namespace FleepBot.Commands
{
	class Items : BaseCommand
	{
		...
		public static Regex regex = new Regex(String.Format("^\\{0}items(?:\\s+(.+))?$", FleepBot.Program.COMMAND_PREFIX), RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline);
