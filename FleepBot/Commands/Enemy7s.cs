		...
		protected override void execute(string convid, string message, string account_id)
		{
			...
			foreach (string guild in guilds)
			{
				...
				int heroLen = Math.Max(6, Math.Max( Math.Max( Math.Max( Math.Max( Math.Max( Math.Max( Math.Max( Math.Max( Math.Max(
									...
									(stats.Item2.Where(x => x.c[0].v.Value == guild).Max(x => x.c[8] != null && x.c[8].v != null ? x.c[8].v.Value.ToString().Trim().Length : 0) ?? 0) + 2),
									(stats.Item2.Where(x => x.c[0].v.Value == guild).Max(x => x.c[10] != null && x.c[10].v != null ? x.c[10].v.Value.ToString().Trim().Length : 0) ?? 0) + 2),
                                    (stats.Item2.Where(x => x.c[0].v.Value == guild).Max(x => x.c[12] != null && x.c[12].v != null ? x.c[12].v.Value.ToString().Trim().Length : 0) ?? 0) + 2),
